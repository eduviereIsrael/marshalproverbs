                        ...
                        try{
                            ...
                            if(response.data.data.status === "cancelled"){
                                const newSnapshot = {...userSnapshot, sub: {
                                  ...
                                  await updateUserSubscription(newSnapshot.id, "Starter", null, null, null, "cancelled")
    ...
    async({userId ,id}, {dispatch, rejectWithValue}) => {
       try{ 
            ...
            if(response?.data?.data?.status === "cancelled"){
                ...
                await updateUserSubscription(userId, "Starter", null, null, null, "cancelled")
...
export const unsubscribeUser = createAsyncThunk(
    "user/unsubscribe",
    async({subCode, email_token}, {dispatch, rejectWithValue}) => {
        try{
            const url = 'https://marshalpayment.netlify.app/api/unsubscribe';
...
const userSlice = createSlice({
    ...
    reducers: {
        ...
        setOverlay: (state, action) => {
            state.unsubOverlay = action.payload;
        },
    ...
    extraReducers:(builder) => {
       ...
       builder.addCase(unsubscribeUser.pending, (state, action) => {
        state.unsubscibedLoading = true;
       })
       builder.addCase(unsubscribeUser.fulfilled, (state, action) => {
        state.unsubOverlay = false;
        state.unsubscibedLoading = false;
       })
...
export const {setUserError, signInSuccess, setOverlay, setLastVisited} = userSlice.actions;
...
export const unsubOverlaySelector = createSelector(
    [selectUser],
    (user) => user.unsubOverlay
)

export const unsubLoadingSelector = createSelector(
    [selectUser],
    (user) => user.unsubscibedLoading
)
