...
export const getPaymentLink = createAsyncThunk(
    ...
    async(cartDetails, {rejectWithValue}) => {
        const url = 'https://marshalpayment.netlify.app/api/acceptpayment';
...
export const verifypayments = createAsyncThunk(
    ...
    async(ref, {rejectWithValue, dispatch}) => {
        const url = `https://marshalpayment.netlify.app/api/verify?reference=${ref}`;
...
export const subscribeUser = createAsyncThunk(
    ...
    async(details, {rejectWithValue, dispatch}) => {
        try{
            ...
            const {data} = await axios.post('https://marshalpayment.netlify.app/api/subscribe', bodyData)
            console.log(data)
            if (data.data.status === "active"){
                console.log("we ran")
                const id = data.data.id
                const response = await updateUserSubscription(userId, productId, id)
